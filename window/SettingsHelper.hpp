...
#include "../util/ShortCutHelper.hpp"
...
inline std::vector<SettingItem> ParseConfig(const std::string& configStr) {
	...
	std::function<void(const nlohmann::json&, std::vector<SettingItem>&)> parseItems =
		...
		for (const nlohmann::basic_json<>& item : items) {
			...
			// 处理expand类型的children
			if (setting.type == "expand" && item.contains("children")) {
...
static void LoadSettingsMap() {
	...
	std::function<void(const std::vector<SettingItem>&)> loadSettingsToMap =
		...
		for (const auto& item : settings) {
			...
			if (item.type == "expand" && !item.children.empty()) {
	...
	std::function<void(std::vector<SettingItem>&, nlohmann::json&)> saveSettingControlValuesSub =
		...
		for (SettingItem& item : settings) {
			...
			if (item.type == "expandswitch") {
				// 通过当前索引来获取hwnd
				HWND hCtrl = nullptr;
				if (HWND parentHwnd = FindTabHwndByName(item.subPage); parentHwnd != nullptr) {
			...
			if (isJustCurrectTab) {
				if (item.subPage != subPageTabs[currentSubPageIndex]) continue;
			}
...
static void initGlobalVariable() {
	...
	PinyinHelper::changePinyinType(g_settings_map["pref_pinyin_mode"].stringValue);
}
...
static void LoadSettingList() {
	...
	for (auto& pair : m_plugins) {
		try {
			...
			SettingItem setting;
			setting.key = wide_to_utf8(pair.plugin->GetPluginPackageName());
			setting.type = "expandswitch";
			setting.title = wide_to_utf8(pair.plugin->GetPluginName());
			setting.subPage = "plugin";
			setting.children = ParseConfig(wide_to_utf8(pair.plugin->DefaultSettingJson()));
	...
	std::vector<SettingItem> appSettings = ParseConfig(settingsText);
		...
		if (!userConfig.is_null() && !userConfig.empty()) {
			...
			std::function<void(std::vector<SettingItem>&)> mergeUserSettings =
				...
				for (auto& setting : settings) {
					...
					// 递归处理expand类型的子项
					if (setting.type == "expand" && !setting.children.empty()) {
