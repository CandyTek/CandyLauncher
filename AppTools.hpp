...
#include "manager/TrayMenuManager.h"
#include "dwmapi.h"
#include "window/SettingWindow.hpp"
#include "util/ShortCutHelper.hpp"
// #include "ListedRunnerPlugin.h"
#include "manager/ListViewManager.h"
#include <atomic>

constexpr bool needOpenDebugCmd = true;
...
static void userSettingsAfterTheAppStart() {
	...
	if (!pref_hotkey_open_with_clipboard_params.empty()) AddHotkey(pref_hotkey_open_with_clipboard_params,
	...
	if (!(g_settings_map["pref_hotkey_show_setting"].stringValue).empty()) AddHotkey(
...
inline void DoMyContextMenuAction(UINT cmd, int index, std::shared_ptr<ActionBase>& action) {
	switch (cmd) {
...
inline void editControlHotkey(WPARAM wParam) {
	std::shared_ptr<ActionBase> it = GetListViewSelectedAction(
	...
	if (!it) {
		...
		case HOTKEY_ID_RUN_ITEM:
			// it->Invoke();
			PluginManager::DispatchActionExecute(it);
			break;
