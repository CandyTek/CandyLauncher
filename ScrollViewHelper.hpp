...
static LRESULT CALLBACK ScrollContainerProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	switch (msg) {
		...
		case WM_COMMAND: {
			...
			if (ctrlId >= 3000 && ctrlId < static_cast<int>(3000 + g_settings2.size())) {
				if (HIWORD(wParam) == BN_CLICKED) {
					...
					if (setting.type == "list") {
						...
						if (setting.key == "pref_skin") {
							HWND g_hSkinComboBox = (HWND) lParam; // 獲取複選框的句柄
							int selIdx = (int) SendMessage(g_hSkinComboBox, CB_GETCURSEL, 0, 0);
							g_currectSkinFilePath = utf8_to_wide(g_settings2[g_prefSkinIndex].entryValues[selIdx]);
