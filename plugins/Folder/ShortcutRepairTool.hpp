		...
		case WM_NOTIFY: {
			...
			if (pnmh->hwndFrom == g_invalidListView && pnmh->code == LVN_ITEMCHANGED) {
				...
				if (pnmv->uNewState & LVIS_SELECTED) {
					...
					// 使用Everything 搜索可能的目标文件
					g_candidates =g_host->SearchPossibleTargets(fileName,50);
...
// 检测快捷方式有效性
static void CheckShortcutValidity(HWND hwnd, std::vector<FileInfo> &fileItems) {
	std::vector<FileInfo> invalidShortcuts;

	// 遍历当前文件集合，检测.lnk文件
	for (const auto &fileInfo: fileItems) {
		...
		if (filePath.size() > 4 &&
			...
			if (IsShortcutInvalid(filePath)) {
				ShowShortcutFixWindow(hwnd,fileItems);
				return;
			}
		}
	}
