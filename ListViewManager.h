...
// 用户遍历的可启动项目和uwp、程序基础快径
inline std::vector<std::shared_ptr<RunCommandAction>> runnerActions;
// 系统正在运行的可见项目
inline std::vector<std::shared_ptr<RunCommandAction>> runningAppActions;
// 用于显示的列表
inline std::vector<std::shared_ptr<RunCommandAction>> filteredActions;
// 模糊搜索的异步线程
...
static void Exactmatch(const std::wstring &keyword) {
	...
	auto addAction = [&](std::vector<std::shared_ptr<RunCommandAction>>& actions) {
	...
	addAction(runningAppActions);
	addAction(runnerActions);
}
...
static void Fuzzymatch_MultiThreaded(const std::wstring &keyword) {
	...
	const size_t totalSize = runnerActions.size();
	...
	for (unsigned int i = 0; i < numThreads; ++i) {
		...
		// 4. 使用安全的索引来创建迭代器
		const auto start_iterator = runnerActions.begin() + start_index;
		const auto end_iterator = runnerActions.begin() + end_index;
	...
	//加载运行中的列表
	for (const auto &action: runningAppActions) {
		if (const double score = rapidfuzz::fuzz::WRatio(lowerKeyword,
														 action->matchText);score >=pref_fuzzy_match_score_threshold) {
			scoredActions.push_back({action, score+8});
		}
	}
...
static void Fuzzymatch_MultiThreaded2(const std::wstring &keyword) {
	...
	// 拷贝快照，保证并行只读
	std::vector<std::shared_ptr<RunCommandAction>> snapshot = runnerActions;
	...
	for (const auto &action: runningAppActions) {
		if (const double score = rapidfuzz::fuzz::WRatio(lowerKeyword,
														 action->matchText);score >=pref_fuzzy_match_score_threshold) {
			scored.push_back({action, score+8});
		}
	}
...
static void Fuzzymatch(const std::wstring &keyword) {
	...
	for (const std::shared_ptr<RunCommandAction> &action: runnerActions) {
...
static void listViewDrawItem(const DRAWITEMSTRUCT *lpDrawItem) {
	...
	if (action->itemType==UWP_APP) {
		if (action->hIcon) { // 假设 action->hIcon 是一个 32-bpp 的 HBITMAP
			// 目标绘制位置
			int iconX;
			int iconY;
			if (isSelected) {
			...
			if (isSelected) {
				...
				ImageList_Draw(g_listFileImageList, unknown_file_icon_index, hdc, rc.left + g_itemIconX, rc.top + g_itemIconY,
							   ILD_TRANSPARENT);
			}
		}
	} else {
		if (g_listFileImageList && action->iImageIndex >= 0) {
			if (isSelected) {
				ImageList_Draw(g_listFileImageList, action->iImageIndex, hdc, rc.left + g_itemIconSelectedX,
							   rc.top + g_itemIconSelectedY,
							   ILD_TRANSPARENT);
			} else {
				ImageList_Draw(g_listFileImageList, action->iImageIndex, hdc, rc.left + g_itemIconX, rc.top + g_itemIconY,
							   ILD_TRANSPARENT);
			}
...
inline std::shared_ptr<ActionNormal> pluginCalcResultAction = std::make_shared<ActionNormal>(L"计算结果", L"",
...
#if defined(DEBUG) || _DEBUG
	int sizeNeeded = WideCharToMultiByte(CP_UTF8, 0, keyword.c_str(), (int)keyword.size(), NULL, 0, NULL, NULL);
	std::string exprStr(sizeNeeded, 0);
	WideCharToMultiByte(CP_UTF8, 0, keyword.c_str(), (int)keyword.size(), &exprStr[0], sizeNeeded, NULL, NULL);
#else
	// ExprTk 使用 std::string，所以需要转换 wstring -> string，英文算式基本都是英文数字，所以直接转换
	std::string exprStr(keyword.begin(), keyword.end());
#endif
...
inline void pluginScientificCalculator(const std::wstring &keyword) {
	filteredActions.clear();
	// 使用 ExprTk 计算结果
	double result = EvaluateExpression(keyword);
	// 转换成字符串
	swprintf(editTextBuf, 64, L"结果: %.15g", result);
	pluginCalcResultAction->SetSubtitle(editTextBuf);
	

	filteredActions.push_back(pluginCalcResultAction);

	// 关键方法，用于提交列表数据更新
	ListView_SetItemCountEx(g_listViewHwnd, 1, LVSICF_NOINVALIDATEALL | LVSICF_NOSCROLL);
	// 设置第一项为选中状态
	ListView_SetItemState(g_listViewHwnd, 0, LVIS_SELECTED | LVIS_FOCUSED, LVIS_SELECTED | LVIS_FOCUSED);
	// 确保第一项可见
	ListView_EnsureVisible(g_listViewHwnd, 0, FALSE);
	ListView_Update(g_listViewHwnd, 0);
	//InvalidateRect(g_listViewHwnd, nullptr, TRUE);
...
static void refreshPluginRunner() {
	// 初始化插件系统
	if (!g_pluginManager) {
		g_pluginManager = std::make_unique<SimplePluginManager>();
		...
		std::string pluginDir = wide_to_utf8(EXE_FOLDER_PATH +L"\\plugins");
	...
	if (g_pluginManager) {
		...
		for (const auto& action : pluginActions) {
			...
			// 调试输出：显示插件动作的标题和匹配文本
			ConsolePrintln(L"Plugin action added - title: " + action.title + L", matchText: " + pluginRunAction->matchText);
...
inline void textMatching() {
	if (MyStartsWith2(editTextBuffer, L"calc ")) {
		// 提取后面的公式，使用ExprTk库，计算出结果
		pluginScientificCalculator(editTextBuffer.substr(5));
	} else {
		// 通知插件用户输入变化
		if (g_pluginManager) {
			std::wstring inputStr = (editTextBuffer);
			g_pluginManager->DispatchUserInput(inputStr);
		}
...
void ExecutePluginAction(const std::wstring& actionId) {
	if (g_pluginManager) {
		std::string actionIdStr = WStringToUtf8(actionId);
