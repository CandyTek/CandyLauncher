...
static bool GetSwitchState(HWND hSwitch) {
	SwitchState *switchInfo = reinterpret_cast<SwitchState*>(GetWindowLongPtr(hSwitch, GWLP_USERDATA));
...
static LRESULT CALLBACK SwitchControlSubclassProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam,
	...
	SwitchState *switchInfo = reinterpret_cast<SwitchState*>(GetWindowLongPtr(hWnd, GWLP_USERDATA));
	...
	switch (uMsg) {
		...
		case WM_NCDESTROY: {
			SwitchState* pSwitchInfo = reinterpret_cast<SwitchState*>(GetWindowLongPtr(hWnd, GWLP_USERDATA));
			if (pSwitchInfo) {
...
static HWND CreateSwitchControl(HWND hParent,size_t subId, int x, int y, int width, int height,
	...
	if (hSwitch) {
		SwitchState* switchInfo = new SwitchState();
