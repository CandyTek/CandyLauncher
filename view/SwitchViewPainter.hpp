...
static void UpdateSwitchAnimation(SwitchState* switchInfo) {
 ...
 * @param switchInfo 
 */
static void SwitchPaintWin8(HWND hWnd, HDC hdc, SwitchState* switchInfo) {
	RECT rect;
	...
	if (switchInfo) {
		...
		InflateRect(&trackRect, -1, -1); // track 填充区（边框内缩 1px）
		RECT realTrackRect = outerRect;
		InflateRect(&realTrackRect, -2, -2); // track 填充区（边框内缩 1px）
 ...
 * @param switchInfo 
 */
static void SwitchPaintWinUi(HWND hWnd, HDC hdc, SwitchState* switchInfo) {
	...
	if (switchInfo) {
		...
		auto GetR = [](COLORREF c) {
			return GetRValue(c);
		};
		auto GetG = [](COLORREF c) {
			return GetGValue(c);
		};
		auto GetB = [](COLORREF c) {
			return GetBValue(c);
		};
		...
		trackPath.AddArc(trackRect.X + trackRect.Width - cornerRadius * 2, trackRect.Y, cornerRadius * 2, cornerRadius * 2, 270, 180);
		// Right semicircle
		trackPath.AddLine(trackRect.X + trackRect.Width - cornerRadius, trackRect.Y + trackRect.Height, trackRect.X + cornerRadius,
						trackRect.Y + trackRect.Height); // Bottom line
 ...
 * @param switchInfo 
 */
static void SwitchPaintRect(HWND hWnd, HDC hdc, SwitchState* switchInfo) {
	RECT rect;
	GetClientRect(hWnd, &rect);
 ...
 * @param switchInfo 
 */
static void SwitchPaintPoorCapsuleShape(HWND hWnd, HDC hdc, SwitchState* switchInfo) {
	RECT rect;
	GetClientRect(hWnd, &rect);
	...
	if (switchInfo) {
		...
		int trackG = static_cast<int>(((trackColorOff >> 8) & 0xFF) + progress * (((trackColorOn >> 8) & 0xFF) - ((trackColorOff >> 8) &
			0xFF)));
		int trackB = static_cast<int>(((trackColorOff >> 16) & 0xFF) + progress * (((trackColorOn >> 16) & 0xFF) - ((trackColorOff >> 16) &
			0xFF)));
		...
		int thumbG = static_cast<int>(((thumbColorOff >> 8) & 0xFF) + progress * (((thumbColorOn >> 8) & 0xFF) - ((thumbColorOff >> 8) &
			0xFF)));
		int thumbB = static_cast<int>(((thumbColorOff >> 16) & 0xFF) + progress * (((thumbColorOn >> 16) & 0xFF) - ((thumbColorOff >> 16) &
			0xFF)));
