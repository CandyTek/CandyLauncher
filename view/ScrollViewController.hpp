	...
	case WM_COMMAND:
		{
			...
			if (ctrlId >= 3000 && ctrlId < static_cast<int>(3000 + g_settings_map.size())) {
				...
				if (setting->pluginId != 65535) {
					m_plugins[setting->pluginId].plugin->OnSettingItemExecute(setting, g_settingsHwnd);
				}
