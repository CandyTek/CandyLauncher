...
static inline UINT GetARGB(Bitmap *bmp, int x, int y) {
	Color c;
	bmp->GetPixel(x, y, &c);
...
static std::vector<Range> ScanRuns(Bitmap *bmp, int x0, int y0, int dx, int dy, int count) {
...
static std::vector<Segment> BuildSegments(int contentLen, const std::vector<Range> &stretchRunsEdge) {
	...
	std::sort(runs.begin(), runs.end(), [](const Range &u, const Range &v) { return u.a < v.a; });
...
static void ParseContentPadding(Bitmap *bmp, int cw, int ch, RECT *outPadding) {
...
RenderNinePatchToSize(const wchar_t *path, int targetW, int targetH, RECT *outContentPadding = nullptr) {

	if (!path || targetW <= 0 || targetH <= 0)
		throw std::invalid_argument("RenderNinePatchToSize: invalid arguments.");
	...
	for (size_t r = 0; r < ySegs.size(); ++r) {
		...
		int dstH = yLens[r];
		if (dstH <= 0 || srcH <= 0) { /* 跳过空块 */ continue; }
		...
		for (size_t c = 0; c < xSegs.size(); ++c) {
			...
			if (c == xSegs.size() - 1 && xSegs[c].e == cw) { adjustedSrcW--; } // 右边缘
			if (r == 0 && ySegs[r].s == 0) {
				adjustedSrcY++;
				adjustedSrcH--;
			} // 上边缘
			if (r == ySegs.size() - 1 && ySegs[r].e == ch) { adjustedSrcH--; } // 下边缘
